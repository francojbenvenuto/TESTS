...
/*____________________________________________________________________
  _________________ ConsultaDesdePerfilSocio _________________________
  ____________________________________________________________________*/

IF OBJECT_ID('facturacion.ConsultarEstadoFacturacionSocio', 'P') IS NOT NULL
    DROP PROCEDURE facturacion.ConsultarEstadoFacturacionSocio;
GO

CREATE PROCEDURE facturacion.ConsultarEstadoFacturacionSocio
    @dni_socio VARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;

    -- Verificación existencia del socio
    DECLARE @id_socio INT;
    ...
    -- Consulta de facturas y saldo actual
    SELECT F.id_factura, F.fecha_emision, F.monto_total, F.estado, F.anulada, S.saldo AS saldo_actual
